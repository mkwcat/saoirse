...
Archive::Archive(const u8* data, u32 size)
  : m_data(data)
  , m_size(size)
{
    m_ok = false;
    ...
    if (m_entriesOffset + m_entryCount * 0xc > m_size ||
        m_entryCount * 0xc > metadataSize) {
        return;
    }
...
std::variant<std::monostate, Archive::File, Archive::Dir> Archive::get(
  u32 entrynum) const
{
    if (!ok()) {
