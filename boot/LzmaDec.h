...
__attribute__((always_inline))
static inline void MY_FAST_CALL LzmaDec_WriteRem(CLzmaDec *p, SizeT limit)
{
...
__attribute__((always_inline))
static inline int MY_FAST_CALL LzmaDec_DecodeReal2(CLzmaDec *p, SizeT limit, const Byte *bufLimit)
{
  if (p->checkDicSize == 0)
  {
...
__attribute__((always_inline))
static inline ELzmaDummy LzmaDec_TryDummy(const CLzmaDec *p, const Byte *buf, const Byte **bufOut)
{
  UInt32 range = p->range;
  UInt32 code = p->code;
...
__attribute__((always_inline))
inline void LzmaDec_InitDicAndState(CLzmaDec *p, BoolInt initDic, BoolInt initState)
{
...
__attribute__((always_inline))
inline SRes LzmaDec_DecodeToDic(CLzmaDec *p, SizeT dicLimit, const Byte *src, SizeT *srcLen,
...
__attribute__((always_inline))
inline SRes LzmaDec_DecodeToBuf(CLzmaDec *p, Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen, ELzmaFinishMode finishMode, ELzmaStatus *status)
{
  ...
  *srcLen = *destLen = 0;
  for (;;)
  {
    ...
    *srcLen += inSizeCur;
    ...
    __builtin_memcpy(dest, p->dic + dicPos, outSizeCur);
    dest += outSizeCur;
    outSize -= outSizeCur;
    *destLen += outSizeCur;
    if (res != 0)
...
__attribute__((always_inline))
inline static void LzmaDec_FreeDict(CLzmaDec *p, ISzAllocPtr alloc)
{
...
__attribute__((always_inline))
inline static SRes LzmaDec_AllocateProbs2(CLzmaDec *p, const CLzmaProps *propNew, ISzAllocPtr alloc)
{
...
__attribute__((always_inline))
SRes inline LzmaDecode(Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen,
    const Byte *propData, unsigned propSize, ELzmaFinishMode finishMode,
    ELzmaStatus *status, ISzAllocPtr alloc)
{
  CLzmaDec p;
